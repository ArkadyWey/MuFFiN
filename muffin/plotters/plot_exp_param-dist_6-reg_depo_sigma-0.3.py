...
# Plot deposition parameter histogram for all N on same graph
# This is not the best algorithm since bars are different widths.
# -----------------------    
...
ax.vlines(x=4*conf.scaled_mean/N, 
              ymin=0.0, 
              ymax=2.0, color="red")
...
# Plot deposition parameter with all N same graph
# Equal sized bars
# -----------------------    
plotting.thesisify_pre_ax_creation()
fig, ax = plt.subplots(1,1)
num_nodes_list = [2*1,2*4,2*16,2*36,2*64,2*100]
colors = ["tab:blue", "tab:orange", "tab:green", "tab:red", "tab:purple", "tab:brown", "tab:pink"]
for t, N in enumerate(num_nodes_list):
...
print(4.0*(conf.scaled_mean/numpy.sqrt(3.0))*conf.get_cdf(conf.mean))
