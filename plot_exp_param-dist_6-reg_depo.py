...
ax.plot(N_smooth, numpy.exp(-0.38)*numpy.power(N_smooth,-1.0/2.0), color="tab:orange", label=r"$0.684N^{-\frac{1}{2}}$",ls="-")
#ax.plot(N_smooth, numpy.exp(-0.25)*numpy.power(N_smooth,-1.0/3.0), color="tab:orange", label=r"$0.472N^{-\frac{1}{3}}$",ls="-")
...
ax.plot( numpy.linspace(1,5,500), -1/2*numpy.linspace(1,5,500) + (-0.38*numpy.ones_like(numpy.linspace(1,5,500))), color="tab:orange", label=r"$-\frac{1}{2}log(N)-0.38$")
